...
mobilenet_path= "ssd_mobilenet_v2_coco_2018_03_29/saved_model"
...
if __name__=="__main__":
    ...
    loaded = tf.saved_model.load(mobilenet_path)
    ...
    while cap.isOpened():
        ...
        detections = int(np.array(result['num_detections'][0]))
        boxes = np.array(result['detection_boxes'][0])[:detections]
        class_names = np.array(result['detection_classes'][0]).astype("bytes")[:detections]
        scores = np.array(result['detection_scores'][0])[:detections]
        ...
        #f_img = draw_boxes(rgb_img, boxes, class_names, scores, max_boxes=10, min_score=prob_tresh)        
                
        #cv2.imwrite("video_frames/frame_%s_%s.jpg"%(i, d), f_img)
        #sys.stdout.buffer.write(f_img)  
        #sys.stdout.flush()
